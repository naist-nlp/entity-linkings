...
def test_CollatorForFEVRY(train: bool) -> None:
    model = FEVRY(retriever=retriever)
    ...
    collator = CollatorForFEVRY(model.tokenizer, dictionary=dictionary, train=train)
    ...
    for batch in dataloader:
        ...
        assert batch["start_positions"].size() == batch["end_positions"].size() == (2, )
