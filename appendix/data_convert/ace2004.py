...
def main(finder: WikiMapper, output_dir: str) -> None:
    output_dir_data = os.path.join(output_dir, 'ace2004')
    os.makedirs(output_dir_data, exist_ok=True)
    print(f"Output directory data: {output_dir_data}")

    data_dir = os.path.join(DownloadManager().download_and_extract(_URL), "gerbil_data", "datasets", "ACE2004_Coref_Turking", "Dev")
    raw_documents = get_documents(os.path.join(data_dir, "RawTextsNoTranscripts"))
    ...
    with open(os.path.join(output_dir_data, "test.jsonl"), 'w', encoding='utf-8') as f:
        for kid, content in raw_documents.items():
            entities = parse_annotation(
                os.path.join(data_dir, "ProblemsNoTranscripts", f"{kid}"), finder
            )
            example = {
                "subset": "ace2004",
                "id": kid,
...
if __name__ == "__main__":
    parser = ArgumentParser(description="ACE2004 Entity Disambiguation Script")
